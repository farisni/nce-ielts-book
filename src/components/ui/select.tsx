...
const SelectContent = forwardRef<HTMLDivElement, SelectContentProps>(
  ({ className, children, scrollFade = true, position: _position, sideOffset: _sideOffset, ..._props }, ref) => {
      ...
      >
        ...
        >
          ...
          <Elevated
            ...
            className={cn(
              `relative flex flex-col gap-0.5 max-h-[300px] overflow-y-auto select-scrollbar ${shape.container} p-1 select-none outline-none`,
              className
            )}
          >
            {/* Selected background */}
...
function SelectValue({ placeholder }: { placeholder?: string }) {
  ...
  return <>{value ? labelMap.current.get(value) ?? value : placeholder}</>;
}
