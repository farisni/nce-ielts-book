...
const defaultItems: SortableItem[] = [
  ...
  { id: "5", title: "Product Additional View", description: "Additional product view", type: "image", size: "3.1 MB" },
]
...
const getTypeColor = (type: SortableItem["type"]) => {
  switch (type) {
    case "image": return "primary-light"
    case "document": return "success-light"
    case "audio": return "destructive-light"
    case "video": return "info-light"
  }
}

export default function DemoSortablePage() {
  const [items, setItems] = useState<SortableItem[]>(defaultItems)

  const handleValueChange = (newItems: SortableItem[]) => {
    setItems(newItems)
    toast.success("排序已更新", {
      description: newItems.map((item, i) => `${i + 1}. ${item.title}`).join(", "),
    })
  }

  return (
    <main className="mx-auto w-full max-w-3xl px-8 py-12">
      <h1 className="text-xl font-semibold mb-8">🧪 Demo · Sortable</h1>
      ...
      >
        {items.map((item) => (
          <SortableItem key={item.id} value={item.id}>
            <div className="bg-background border-border hover:bg-accent/50 rounded-md flex cursor-pointer items-center gap-3 border p-3 transition-colors">
