...
import { PHONICS_MIXED_CHAPTERS } from "@/lib/speller/phonics-mixed";
import type { Course } from "@/lib/speller/courses";

/**
 * Speller · 元辅组合听写页
 * 按分组整体学习（a 特殊词形 / i 特殊词形 / c·g 变音 / o 特殊词形 / 常见词尾），
 * 进度按分组记忆（phonics-mixed-*）；每个单词带所属组合的读音（phoneticFocus），
 * 显示音标+释义，答对后对应组合字母红色突出
 */
export default function MixedDictationPage() {
  const router = useRouter();
  const params = useParams();
  const chapterSlug = (params.chapter as string) ?? "mixed-a";
  const chapter = PHONICS_MIXED_CHAPTERS.find((c) => c.slug === chapterSlug);
  ...
  // 进度记忆按分组（restoreKey = phonics-mixed-${chapter}）
  const course: Course = {
    id: `phonics-mixed-${chapterSlug}`,
    name: `${chapter.name} · 元辅组合听写`,
    description: `元辅组合 · ${chapter.tests.length} 个规则 · ${words.length} 词`,
      ...
      <div className="flex min-h-0 w-full flex-1 flex-col items-center justify-center px-6 py-6">
        <SentencePractice
          ...
          onNextGroup={() => {
            ...
            const slugs = PHONICS_MIXED_CHAPTERS.map((c) => c.slug);
            const next = slugs[(slugs.indexOf(chapterSlug) + 1) % slugs.length];
            router.push(`/speller/phonics-mixed-dictation/${next}`);
          }}
