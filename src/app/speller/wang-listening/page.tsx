...
export default function WhaleListeningPage() {
  ...
  useEffect(() => {
    try {
      const saved = window.localStorage.getItem("speller:wang-chapter");
      if (saved && WHALE_CHAPTERS.some((c) => c.slug === saved)) {
  ...
  const toggleChapter = (slug: string, isOpen: boolean) => {
    ...
    try {
      window.localStorage.setItem("speller:wang-chapter", slug);
    } catch {
      // ignore
    }
  };
