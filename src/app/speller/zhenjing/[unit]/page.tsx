...
import { getZhenjingUnit } from "@/lib/speller/zhenjing";
 ...
 */
export default function ZhenjingDictationPage() {
  const router = useRouter();
  const params = useParams();
  const unitSlug = (params.unit as string) ?? "unit-1";
  const unit = getZhenjingUnit(unitSlug);
  ...
  if (!unit) {
    return (
      <div className="flex h-full w-full flex-col items-center justify-center gap-4 text-center">
        <p className="text-lg text-muted-foreground">Unit 不存在</p>
      </div>
    );
  }

  // 动态构造听写课程：单词 → 句子条目（en=单词, cn=释义, phonetic=音标）
  // sessionSize = 整个 Unit 的词数：一次练完全部单词，进度条反映 Unit 真实进度
  // 音标：部分源数据用 ASCII 撇号 ' 表重音，转标准 IPA ˈ；// 包围由 SentencePractice 统一处理
  const course: Course = {
    id: `zhenjing-${unitSlug}`,
    name: `雅思词汇真经 · ${unit.name}`,
    description: `雅思词汇真经单词听写 · ${unit.words.length} 词`,
    ...
    <div className="flex h-full min-h-0 flex-1 flex-col">
      ...
      <div className="flex min-h-0 w-full flex-1 flex-col items-center justify-center px-6 py-6">
        <SentencePractice
          ...
          onExit={() => router.push("/speller/zhenjing")}
        />
      </div>
