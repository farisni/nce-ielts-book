#!/usr/bin/env python3
"""
为辅音组合听写课程（phonics-consonant.ts，150 词）抓取原声（哇学社 tts 接口，Edge TTS 合成），
下载到 public/audio/consonant/{chapter-slug}/{word}.mp3，并写入数据文件 audio 字段。

- POST api.waxueshe.com/api/v2/phonetic/tts  {text, voice}
- voice: uk_female（英式）
- 限速 0.3s，失败自动重试 3 次
用法：python3 scripts/waxue-consonant-audio.py [--token <TOKEN>]
"""
import json
import os
import re
...
VOICE = "uk_female"
DATA = "src/lib/speller/phonics-consonant.ts"
AUDIO_DIR = "public/audio/consonant"
...
def main():
    ...
    for i, line in enumerate(lines):
        sm = re.search(r'slug: "(consonant-[a-z]+)"', line)
        if sm:
            cur_slug = sm.group(1)
    ...
    for idx, (lineno, line, word, slug) in enumerate(word_lines):
        ...
        if os.path.exists(path):
            ...
            if not download(url, path):
                ...
                continue
            ok += 1
        audio_url = f"/audio/consonant/{slug}/{safe}.mp3"
        if 'audio: ""' in line:
